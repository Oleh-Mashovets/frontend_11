...
const parsedData = JSON.parse(objectList);
const wrap = document.getElementById('character__list');

function addElement () {
   for(let user = 0; user < parsedData.results.length; user++) {
   let newDiv = document.createElement(`div`);
   newDiv.classList.add('item__block');
   newDiv.innerHTML = `<div class="wrapper"><div class="wrapper__item"><p class="item__text">${parsedData.results[user]?.id}</p><p class="item__text">${parsedData.results[user]?.name}</p><p class="item__text">${parsedData.results[user]?.status}</p></div><div class="wrapper__button"><button btn-name="delete" onClick="handleRecordAction(event)" id="btn">Delete</button></div></div>`;
   ...
   pGet.innerHTML = parsedData.results[user].name;
   ...
   newDiv.classList.add('toggle__border');
      ...
      //       let c = 0;
      //       while (c < el.length) {
      //          el[c++].className = 'item__block';
      //          }
      //          el[i].className = ' toggle__border item__block';
      //       }
      //    }  
   })
   }
...
btnList.appendChild(btnShow);

btnShow.addEventListener('click', () => {
   if(document.querySelectorAll('.item__block').length === 0) {
      addElement();
   }
})